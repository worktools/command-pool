...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..args.concurrency.min(args.total_tasks) {
        ...
        join_set.spawn(async move {
            ...
            println!("[Task {}] Starting... (Running: {})", task_id, running_tasks_clone.load(Ordering::SeqCst));
            ...
            let (result_msg, stdout_output, stderr_output) = match output_result {
                Ok(output) => {
                    ...
                    if output.status.success() {
                        ...
                        (format!("Success (Exit Code: {})", output.status.code().unwrap_or_default()), stdout, stderr)
                    } else {
                        failed_tasks_clone.fetch_add(1, Ordering::SeqCst);
                        failed_durations_clone.lock().unwrap().push(task_duration); // Store duration
                        (format!("Failed (Exit Code: {})", output.status.code().unwrap_or_default()), stdout, stderr)
                    }
                },
                Err(e) => {
                    failed_tasks_clone.fetch_add(1, Ordering::SeqCst);
            ...
            println!("[Task {}] Finished: {} (Running: {})", task_id, result_msg, running_tasks_clone.load(Ordering::SeqCst));
            if !quiet_clone && !stdout_output.is_empty() {
                println!("[Task {}] Stdout:
{}", task_id, stdout_output);
            }
            if !stderr_output.is_empty() {
                eprintln!("[Task {}] Stderr:
...
{}", task_id, stderr_output);
    ...
    println!("\nTotal command-pool execution time: {}", humantime::format_duration(total_duration));
